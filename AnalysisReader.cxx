...
EL::StatusCode AnalysisReader :: histInitialize_monoWZH()
{
  ...
  double min;
  double max;
  static std::string cuts [11] = {"All", "pre-selection",  "250met", "350met", "one fatjet", "no elec", "no muon", "no lepton", "addjetveto", "500met" ,"mJ"};
  //pre-selection
...
{ 
  ...
  if(selElectrons.size() == 0) pass_noelec = true;
  if(selElectrons.size() == 0 && selMuons.size() == 0) pass_nomuon = true;
  ...
  if(pass_250met && pass_350met && pass_fatjet && pass_noelec)
  {
        m_hist_mono_cutflow->Fill(m_hist_mono_cutflow->GetXaxis()->FindBin("no elec"), m_weight);
        m_hist_mono_cutflow_noweight->Fill(m_hist_mono_cutflow_noweight->GetXaxis()->FindBin("no elec"));
  }
  if(pass_250met && pass_350met && pass_fatjet && pass_noelec && pass_nomuon)
  {
