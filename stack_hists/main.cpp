...
int main (int argc, char * argv[])
{
	...
	// AB colour scheme
	//             Zll,     Wenu,    Wmunu,   Wtaunu,  ttbarall, ttbarnon, singlet,  gammajet,   dijet,  Zvv,       signal
        //int color[] = {kBlue-6, kCyan-2, kCyan-6, kCyan-9, kGreen+2, kGreen-3, kGreen-7, kMagenta+3, kRed-9, kOrange+8, kRed+2};
	// YT colour scheme
	...
	// remove single top
	//             ttbarall nonall,  Wlv(ev /    muv /      taumu),     Zll,        Zvv         gamma    dijet  monoW monoH
	int color[] = {kOrange, kOrange, kGreen - 9, kGreen - 4,kGreen - 2, kAzure + 2, kAzure - 4, kYellow, kCyan, kRed, kBlue};
	...
	vector<TH1F*> error;
	vector<TGraphAsymmErrors*> hatched_error;
	...
	{   if(pass < 3)
		...
		not_stacked_hists.push_back(new THStack(first_key->GetName(), title));
		error.push_back(new TH1F(first_key->GetName(), title,hist->GetXaxis()->GetNbins(),hist->GetXaxis()->GetXmin(),hist->GetXaxis()->GetXmax()));
		hatched_error.push_back(new TGraphAsymmErrors(hist->GetXaxis()->GetNbins()));
	...
	if(TFflag){
		...
		for (int k = 0; k < stacked_hists.size(); k++)
		{
			...
			stacked_hists.at(k)->Draw("nostack hist");
			...
			not_stacked_hists.at(k)->Draw("nostack hist");

			char eps_file_name[256];
			sprintf(eps_file_name, "%s_%s.eps", argv[1], stacked_hists.at(k)->GetName());
		...
		for (int k = 0; k < stacked_hists.size(); k++)
		{
			...
			// fill TGraphAssymErrors
			gStyle->SetHatchesSpacing(0.5);
   			gStyle->SetHatchesLineWidth(1);	
			hatched_error.at(k)->SetFillStyle(3354);
    			hatched_error.at(k)->SetFillColor(1);
			int n_bins = error.at(k)->GetNbinsX();
    			for (unsigned int n = 0; n < n_bins; ++n) {
        			hatched_error.at(k)->SetPoint(n, error.at(k)->GetBinCenter(n + 1), error.at(k)->GetBinContent(n + 1));
        			hatched_error.at(k)->SetPointError(n, error.at(k)->GetBinWidth(n + 1) / 2., error.at(k)->GetBinWidth(n + 1) / 2.,
              						       	      error.at(k)->GetBinError(n + 1), error.at(k)->GetBinError(n + 1));
    			}		
